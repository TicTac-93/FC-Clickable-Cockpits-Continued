...
self:listen_command(3001)
self:listen_command(24)
...
function SetCommand(command, value)
  ...
  elseif command == device_commands.ADI_RESET then
    if value > 0 then
      ...
      dispatch_action(24, iCommands.SYS_F86_ResetADI, 1)
    else
      dispatch_action(24, iCommands.SYS_F86_ResetADI, 0)
    end
...
function adi_reset_handler()
  if adi_reset < 0 then
    return
  elseif adi_reset == 1 then
    -- This command has to be sent to device 24, guess it's not listening for the command code
    dispatch_action(24, iCommands.SYS_F86_ResetADI, 1)
  elseif adi_reset == 0 then
    dispatch_action(24, iCommands.SYS_F86_ResetADI, 0)
    adi_reset = -1
  end
end
